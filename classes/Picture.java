...
public class Picture extends SimplePicture {
	...
	public void mirrorArms() {
		...
		for (int row = 155; row < 200; row++) {
			for (int col = 95; col < mirrorPoint; col++) {
				leftPixel = pixels[row][col];
		...
		for (int col = 95; col < 294; col++) {
			for (int row = 160; row < 160 + 68; row++) {
				topPixel = pixels[row][col];
				bottomPixel = pixels[(160 + 30) * 2 - 1 - row][col];
				bottomPixel.setColor(topPixel.getColor());
			}
		}
	}
	...
	public void mirrorGull() {
		...
		for (int row = 235; row < 322; row++) {
			for (int col = 220; col < 350; col++) {
				leftPixel = pixels[row][col];
				rightPixel = pixels[row - 9][col - 150];
				rightPixel.setColor(leftPixel.getColor());
			}
		}
	}
	...
	public void copy(Picture fromPic, int startRow, int startCol, int endRow,
			int endCol) {
		Pixel fromPixel = null;
		Pixel toPixel = null;
		Pixel[][] toPixels = this.getPixels2D();
	...
	public void grayscale() {
		...
		for (Pixel[] rowArray : pixels) {
			for (Pixel pixelObj : rowArray) {
				pixelObj.setRed((pixelObj.getRed() + pixelObj.getGreen() + pixelObj
						.getBlue()) / 3);
				pixelObj.setGreen((pixelObj.getRed() + pixelObj.getGreen() + pixelObj
						.getBlue()) / 3);
				pixelObj.setBlue((pixelObj.getRed() + pixelObj.getGreen() + pixelObj
						.getBlue()) / 3);
			}
		}
	}
